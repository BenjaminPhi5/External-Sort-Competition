...
public class CountingSortBytes {

    // The size of the buffer used for reading data.
    private static final int BUF_SIZE = 1 << 16;
    // The size of the array used to store repeats (determined experimentally).
    private static final int ARR_SIZE = 5002500;
    ...
    // The main run method for the algorithm.
    public static void run(String filenameIn, RandomAccessFile fos, int min, int max) {
        // This function takes the name of the file to read in, and the object used for an output stream,
        // as well as the min and max values to sort on the data.

        // This version of counting sort assumes low numbers of repeats, hence using a byte to store the
        // number of repeats if each element, and offsets each value by a minimum (to prevent any negative indexes).
        ...
        // deallocate arrays now that they are finished with.
        arr = null;
        buf = null;

    }

    private static void readFromBuffer(int boundLower, int boundUpper) throws IOException{
        ...
        while ((n = fis.read(buf)) != READ_FINISHED) {
            ...
            for (int i = 0; i < intsRead; i++) {
                ...
                // check if the current value fits in the given bound, and if so increment the number of
                // repeats for that element.
